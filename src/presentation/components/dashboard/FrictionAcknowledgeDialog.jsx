...
export function FrictionAcknowledgeDialog({ friction, onSave, onClose, open }) {
  const existing = friction || {};
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await onSave(existing.friction, {
        confirmed,
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{existing.label || existing.friction}</DialogTitle>
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="friction-confirmed" className="text-sm font-medium leading-snug">
              ¿Reconoces este patrón en ti?
            </Label>
            <Switch
              id="friction-confirmed"
              checked={confirmed}
          ...
          {confirmed && (
            <>
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="friction-working" className="text-sm font-medium leading-snug">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="friction-notes" className="text-sm font-medium">
